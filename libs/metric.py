...
def get_segments( #change frame-level label to segment-level label
    ...
    if last_label != bg_class: #if the last frame is not background
        ends.append(i)
...
class ScoreMeter(object):
    ...
    def _fast_hist(self, pred: np.ndarray, gt: np.ndarray) -> np.ndarray:
        ...
        ).reshape(self.n_classes, self.n_classes) #confusion matrix
        return hist
    ...
    def update(
        ...
        for pred, gt in zip(preds, gts):
            ...
            self.n_frames += len(pred) #total_frame_num
            ...
            for i, th in enumerate(self.iou_thresholds): #f1-socre at IOU of 0.1,0.25,0.5,0.75,0.9
    ...
    def update(self, preds, gts, masks):
        ...
        for pred, gt, mask in zip(preds, gts, masks):
            ...
            for i in range(len(pred_idx)):
                ...
                if min_dist <= self.tolerance and hits[idx] == 0: #shift < 5
                    tp += 1 #tp
                    hits[idx] = 1
                else:
                    fp += 1
